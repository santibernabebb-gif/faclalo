...
// We'll use CDN loaded scripts for docxtemplater and jspdf
// This prevents complex bundling issues in a single-file-like response environment
...
export async function generateDocx(budget: BudgetData, config: InvoiceConfig, invoiceCode: string) {
  ...
  try {
    ...
    // Note: Template keys should match {cliente}, {fecha}, {invoiceCode}, etc.
    // The prompt says "NO cambiar diseño, solo rellenar" and "Todo lo que se rellene debe ir en NEGRITA".
    // Usually docxtemplater respects the formatting of the tag in the docx.
    doc.setData({
      invoice_code: invoiceCode,
      cliente: budget.clientName,
      fecha: config.date.split('-').reverse().join('/'), // Convert to DD/MM/YYYY
